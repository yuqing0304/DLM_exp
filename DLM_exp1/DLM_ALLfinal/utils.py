...
# addtional_dataset_filename
    parser.add_argument(
        "--additional_dataset_filename",
        type=str,
        default="additional_phrase.txt",
        help="additional_dataset_filename",
    )

    parser.add_argument(
        "--train_dataset_filename",
        type=str,
        default="trainmeaning_phrase.txt",
        help="Dataset filename",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="testmeaning_phrase.txt",
        help="Dataset filename",
    )
    ...
    parser.add_argument(
        "--listener_embedding_size", type=int, default=8, help="listener_embedding_size"
    )
    ...
    parser.add_argument(
        "--word_dropout_p", type=float, default=0.1, help="word_dropout_p"
    )

    parser.add_argument(
        "--spk_entropy_coeff", type=float, default=0.1, help="spk_entropy_coeff"
    )

    # parser.add_argument(
    #     "--device", type=str, default='cpu', help="early_stopping_patience"
    # )
...
def cutting_length(message, receiver_output, labels):
    # ensure that the message and receiver output have the same length as the ground-truth target sequence, which is necessary for computing the loss during training
