...
class ConsoleLogSaver(ConsoleLogger):
    ...
    def on_train_ending(self, loss: float, logs: Interaction, epoch: int):
        ...
        if epoch % self.dump_every == 0 or epoch < 10:
            dump_dir_mean = os.path.join(self.outputs_dir, f'mean_epoch{epoch}_train')
            dump_dir_uttr = os.path.join(self.outputs_dir, f'uttr_epoch{epoch}_train')
            dump_dir_msg = os.path.join(self.outputs_dir, f'msg_epoch{epoch}_train')
            dump_dir_lstpredict = os.path.join(self.outputs_dir, f'lstpred_epoch{epoch}_train')
    ...
    def on_validation_end(self, loss: float, logs: Interaction, epoch: int):
        ...
        if epoch % self.dump_every == 0 or epoch < 10:
            dump_dir_mean = os.path.join(self.outputs_dir, f'mean_epoch{epoch}_test')
            dump_dir_uttr = os.path.join(self.outputs_dir, f'uttr_epoch{epoch}_test')
            dump_dir_msg = os.path.join(self.outputs_dir, f'msg_epoch{epoch}_test')
            dump_dir_lstpredict = os.path.join(self.outputs_dir, f'lstpred_epoch{epoch}_test')
            torch.save(logs.sender_input, dump_dir_mean)
            torch.save(logs.labels, dump_dir_uttr)
            torch.save(logs.message, dump_dir_msg)
            torch.save(logs.receiver_output, dump_dir_lstpredict)
