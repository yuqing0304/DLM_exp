...
bars1 = ax.bar(x - 1.5*width, avg_percent_short_per_phase_1, width, yerr=ci_95_per_phase_1, capsize=5, color='#dbe9f6', label='Baseline') # 'Baseline communication'

# Bars for noise 0.1 - light blue
bars2 = ax.bar(x - 0.5*width, avg_percent_short_per_phase_2, width, yerr=ci_95_per_phase_2, capsize=5, color='#b3d3ea', label='Noise 0.1')

# Bars for lower spk capacity - slightly darker light blue
bars3 = ax.bar(x + 0.5*width, avg_percent_short_per_phase_3, width, yerr=ci_95_per_phase_3, capsize=5, color='#8cbede', label='Lower spk capacity')

# Bars for lower spk capacity and noise 0.1 - medium-light blue
bars4 = ax.bar(x + 1.5*width, avg_percent_short_per_phase_4, width, yerr=ci_95_per_phase_4, capsize=5, color='#6499b7', label='Lower spk capacity & noise 0.1')
...
ax.set_title('Verb Initial, Skewed Mixed', fontsize=16)
ax.set_xticks(x)
...
plt.tight_layout()
plt.savefig('comm_initial_rnn.pdf', dpi=300)  # Save with higher resolution
plt.show()
