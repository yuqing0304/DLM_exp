...
def main(params: List[str], suffix) -> None:
    ...
    opts.listener_embedding_size = 256 # 512 # 256
    ...
    trainer_lst.train(n_epochs=opts.n_lstepochs)
    ...
    trainer_spk.train(n_epochs=opts.n_spkepochs)
    ...
    optimizer = core.build_optimizer(game_selfplay.parameters(), lr=0.00001)
    optimizer_scheduler = None
    ...
    trainer_selfplay.train(n_epochs=opts.n_commepochs)
...
if __name__ == "__main__":
    ...
    # meaning_embedding_dim = 128
    hyperdict = {'language': ['final_skewed'], 'trainset_proportion': [0.667], 'speaker_hidden_size': [1024], 'listener_hidden_size': [1024],'meaning_embedding_dim': [128], 'spk_entropy_coeff': [0.1], 'batch_size': [32], 'word_dropout_p': [0], 'random_seed': [
