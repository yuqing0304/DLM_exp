...
class MyLoss_lst:
    ...
    def __call__(
        ...
        # print(f"sender_input: {sender_input}")
        ...
        eq_objectm1 = torch.eq(message[:, 6], labels[:, 6])
        eq_objectm2 = torch.eq(message[:, 7], labels[:, 7])
        eq_objectm3 = torch.eq(message[:, 8], labels[:, 8])
        ...
        # return accumulate_loss, {"acc": acc, "acc_verb": acc_verb, "acc_subject": acc_subject, "acc_subjectm1": acc_subjectm1}