...
#         in_features = embedding_size * max_len
#         self.activation_fn = activation_fn
#         self.gcn = GCNConv(in_features, output_size)
#         # self.fc_out = nn.Linear(in_features, output_size)
#         self._edge_index = torch.tensor([
#                             [1, 0],  # cat -> chase (subject-verb)
#                             [5, 0],  # mouse -> chase (object-verb)
#                             [2, 1],  # white -> cat (adjective-noun)
#                             [3, 1],  # fur -> cat (noun-noun)
#                             [4, 1],  # with -> mouse (preposition-noun)
#                             ], dtype=torch.long).t().contiguous()
#         # chase cat with white fur mouse
#         # 0      1    2    3    4     5
#     def forward(
#             self,
#             sender_input: torch.Tensor,
...
#         # context = self.activation_fn(self.fc_out(embedded_concat))
#         # context = self.fc_out(embedded_concat)
#         # self._edge_index.to(sender_input.device)
#         # print(f"input: {sender_input.device}, edge_index: {self._edge_index.device}")
#         context = self.gcn(embedded_concat, self._edge_index.to(sender_input.device))
#         return context
