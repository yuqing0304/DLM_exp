...
def remove_elements(enc_out, sender_input, value_to_check):
    ...
    for i in missing_indices:
        indices_to_remove = torch.cat([indices_to_remove, torch.tensor([[i, -1]], device=device)], dim=0)
...
class Listener_encoder(EncoderRNN):
    ...
    # def __init__(self, vocab_size, max_len, hidden_size, word_dropout_p, dropout_constant, rnn_cell, variable_lengths, sos_id, eos_id, pad_id):
    #     # super(Listener_encoder, self).__init__(*args, **kwargs)
    #     super(Listener_encoder, self).__init__(vocab_size, max_len, hidden_size, word_dropout_p, dropout_constant, rnn_cell, variable_lengths)
    ...
    def forward(self, input_var, input_lengths=None, token_to_remove=None):
        # return super().forward(input_var, input_lengths, token_to_remove)
...
# ref from reinforce_wrappers.py:
...
class Listener(nn.Module):
    ...
    def forward(
        ...
        enc_out, uttr_representation = self.encoder(sender_input, input_length, token_to_remove=None)
        ...
        # add options: eval or train
        ## add each step loss for listener
        step_outs = []
        # step_preds = []
        ...
        # else:        
        #     # print('==============eval mode===============')
        #     listener_output, prediction = self.decoder(enc_out[:, 7, :].squeeze(1))
        #     # prediction (as message) is used to calculate accuracy
        #     # 1, 2, 3, 4, 5, 6, 7, 8 [0:SOS, 9:EOS]
        for i in range(enc_out.size(1)):
            step_out, step_pred = self.decoder(enc_out[:, i, :].squeeze(1))
            # print(f"step_out: {step_out.shape}")
            step_outs.append(step_out) # step_outs are used to calculate avg loss
            # step_preds.append(step_pred) # step_preds are used to calculate accuracy
                
        logits = torch.zeros(listener_output.size(0)).to(listener_output.device)
        entropy = logits

        # return step_outs, step_preds[-1], logits, entropy
        return step_outs, prediction, logits, entropy
